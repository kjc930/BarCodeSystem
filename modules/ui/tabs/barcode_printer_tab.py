    ...
    def refresh_ports(self):
        ...
        for port in ports:
            try:
                ...
                # 기타 예외 처리
                unavailable_ports.append((port, str(e)))
                print(f"DEBUG: 프린터 포트 테스트 중 예외: {port.device} - {e}")
                # print(f"DEBUG: 포트 테스트 중 예외: {port.device} - {e}")
                time.sleep(0.2)
                continue
        
        print(f"DEBUG: 프린터 사용 가능한 포트 수: {len(available_ports)}")
